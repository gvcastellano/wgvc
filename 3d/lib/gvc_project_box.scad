...
// Screw dimensions to hold case closed
module case_screw () {
  screw_hole(DIN963, M3, 25, 3);
}

case_screw_inset = 4;
// x=y distance from corners of box

case_screw_mount_w = 3;


with_pcb_mounts = 1;
pcb_mount_x_size = 30;
pcb_mount_y_size = 50;
pcb_mount_x_offset = 5;
pcb_mount_y_offset = 5;
pcb_mount_w = 3;
pcb_mount_h = 3;
pcb_mount_d = 2;

with_bottom_rj45_port = 1;
bottom_rj45_port_x_offset = 5;
bottom_rj45_port_y_offset = pcb_mount_y_size+pcb_mount_x_offset+1;
bottom_rj45_port_x_size = 5;
bottom_rj45_port_y_size = 6;
...
cover_allowance = 2;
// For fitting cover to lower box

upper_box_height = 10;
//Includes owt+cover_allowance
...
// don't change
lower_box = [pcb.x+with_clamping*2*clamp_size+2*iwt+2*owt,
             pcb.y+with_clamping*2*clamp_size+2*iwt+2*owt,
             pcb.z/2+space_below+owt];

upper_box = [lower_box.x,
             lower_box.y,
             upper_box_height];
//orig             owt+pcb.z -(pcb_height+cable_drills)];

upper_box_translate = [lower_box.x+upper_box.x/2,0,0];

if(generate_upper){
    translate(upper_box_translate) {  // comment this to flip the box on top of each other for testing
        ...
        union(){
            difference(){
                ...
                union(){
                    ...
                    translate([-upper_box.x/2+1/2*owt-shrinkage,
                               -upper_box.y/2+1/2*owt-shrinkage,
                               upper_box_height-cover_allowance]) {
// orig                               owt+1/2*pcb.z]) {
                        cube([upper_box.x-owt+2*shrinkage,
                              upper_box.y-owt+2*shrinkage,
                              cover_allowance]);
// orig                              1/4*pcb.z+space_below/2]);
                    }
                    ...
                    // drills in the sizes
                    for(a = y_drills_bot){
                        translate([-a,-upper_box.y/2+owt,upper_box_height]){
                            rotate([90,0,0]){
                    ...
                    for(b = y_drills_top){
                        translate([-b,+upper_box.y/2,upper_box_height]){
                            rotate([90,0,0]){
                                cylinder(h=owt, r=cable_drills);
                            }
                        }
                    ...
                    for(b = x_drills_left){
                        translate([upper_box.x/2-owt,b,upper_box_height]){
                            rotate([0,90,0]){
                                cylinder(h=owt, r=cable_drills);
                            }
                        }
                    ...
                    for(b = x_drills_right){
                        translate([-upper_box.x/2,b,upper_box_height]){
                            rotate([0,90,0]){
                                cylinder(h=owt, r=cable_drills);
                            }
                        }
                    ...
                    if (with_case_screws) {
                      translate([+upper_box.x/2-case_screw_inset, 
                                 +upper_box.y/2-case_screw_inset, 
                                 0]) case_screw();
                      translate([-upper_box.x/2+case_screw_inset, 
                                 +upper_box.y/2-case_screw_inset, 
                                 0]) case_screw();
                ...
                } // union of all things subbtracted from the upper base cube
            }
        } 
...
if(generate_lower){
    ...
    union(){
        difference(){ // base box - (inner cube - walls around)
            ...
            union(){
                // inner cube
                translate([-(lower_box.x/2-owt),
                           -(lower_box.y/2-owt),
                           owt]) {
                    cube([lower_box.x-owt*2,
                          lower_box.y-owt*2,
                          lower_box.z-owt]);
                             // lower_box.z = pcb.z/2+space_below+owt
                }
                // remove 4 half - walls
                translate([-lower_box.x/2,
                           -lower_box.y/2,
                           lower_box.z - cover_allowance]) {
// orig                            space_below+pcb_height+owt+cable_drills]) {
                    cube([lower_box.x,owt/2,cover_allowance]);
                }
                translate([-lower_box.x/2,
                           lower_box.y/2-owt/2,
                           lower_box.z - cover_allowance]) {
                ...
                translate([-lower_box.x/2,
                           -lower_box.y/2-owt/2,
                           lower_box.z - cover_allowance]) {
                    cube([owt/2,lower_box.y,cover_allowance]);
                }
                translate([lower_box.x/2-owt/2,
                           -lower_box.y/2-owt/2,
                           lower_box.z - cover_allowance]) {
                    cube([owt/2,lower_box.y,cover_allowance]);
                }
                ...
                for(a = y_drills_bot){
                    translate([a,
                               -lower_box.y/2+owt,
                               lower_box.z-cover_allowance]){
// orig                               cable_drills+space_below+pcb_height+owt]){
                        rotate([90,0,0]){
                            cylinder(h=owt, r=cable_drills);
                        }
                    }
                ...
                for(b = y_drills_top){
                    translate([b,+lower_box.y/2,lower_box.z-cover_allowance]){
                        rotate([90,0,0]){
                            cylinder(h=owt, r=cable_drills);
                        }
                    }
                ...
                for(b = x_drills_left){
                    translate([-lower_box.x/2,b,lower_box.z-cover_allowance]){
                        rotate([0,90,0]){
                            cylinder(h=owt, r=cable_drills);
                        }
                    }
                ...
                for(b = x_drills_right){
                    translate([lower_box.x/2-owt,b,lower_box.z-cover_allowance]){
                        rotate([0,90,0]){
                            cylinder(h=owt, r=cable_drills);
                        }
                    }
                ...
                if (with_bottom_rj45_port) {
                  translate([-lower_box.x/2+bottom_rj45_port_x_offset, 
                             -lower_box.y/2+bottom_rj45_port_y_offset,
                             0])
                     cube([bottom_rj45_port_x_size, bottom_rj45_port_y_size, owt]);   
                }
            } // end of union
        ...
        if (with_case_screws) {
          translate([+upper_box.x/2-case_screw_inset, 
                     +upper_box.y/2-case_screw_inset, 
                     owt]) prism_mount(h=lower_box.z-owt, w=case_screw_mount_w, d=1);
          translate([-upper_box.x/2+case_screw_inset, 
                     +upper_box.y/2-case_screw_inset, 
                     owt]) prism_mount(h=lower_box.z-owt, w=case_screw_mount_w, d=1);
          translate([-upper_box.x/2+case_screw_inset, 
        ...
        if (with_pcb_mounts) {
          translate([-upper_box.x/2+pcb_mount_x_offset, 
                     -upper_box.y/2+pcb_mount_y_offset, 
                     owt]) cube_mount(h=pcb_mount_h, pcb_mount_d=1);
          translate([-upper_box.x/2+pcb_mount_x_offset+pcb_mount_x_size, 
                     -upper_box.y/2+pcb_mount_y_offset, 
                     owt]) cube_mount(h=pcb_mount_h, pcb_mount_d=1);
          translate([-upper_box.x/2+pcb_mount_x_offset, 
                     -upper_box.y/2+pcb_mount_y_offset+pcb_mount_y_size, 
                     owt]) cube_mount(h=pcb_mount_h, pcb_mount_d=1);
          translate([-upper_box.x/2+pcb_mount_x_offset+pcb_mount_x_size, 
                     -upper_box.y/2+pcb_mount_y_offset+pcb_mount_y_size, 
                     owt]) cube_mount(h=pcb_mount_h, pcb_mount_d=1);
